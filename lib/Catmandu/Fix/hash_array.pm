...
sub _build_array_fields {
    ...
    for my $pub_type (keys %$forms) {
        ...
        my $supplementary_fields
            = $forms->{$pub_type}->{fields}->{supplementary_fields};
        my $related_material
            = $forms->{$pub_type}->{fields}->{related_material};

        for my $field (keys %$basic_fields) {
            my $multiple = $basic_fields->{$field}->{hash_array}
        ...
        for my $field (keys %$supplementary_fields) {
            my $multiple = $supplementary_fields->{$field}->{hash_array}
                // $supplementary_fields->{$field}->{multiple} // 0;
            $lookup->{$pub_type}->{$field} = $multiple;
        }

        for my $field (keys %$related_material) {
            my $multiple = $related_material->{$field}->{hash_array}
