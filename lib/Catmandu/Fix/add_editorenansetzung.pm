...
sub fix {
        ...
        and $pub->{editor}->[0])
    {
        ...
        foreach my $editor (@{$pub->{editor}}) {
            ...
            my $first_initial = substr $editor->{first_name}, 0, 1
                if $editor->{first_name};

            my @initials = split " ", $editor->{first_name}
                if $editor->{first_name};
            foreach (@initials) {
            ...
            my @initials_dot = split " ", $editor->{first_name}
                if $editor->{first_name};
            ...
            # "Müller, Karl Heinz"
            push @{$editor->{editoren_ansetzung}}, "$editor->{full_name}"
                if $editor->{full_name};

            # "Kaufmann"
            # "Müller"
            push @{$editor->{editoren_ansetzung}}, "$editor->{last_name}"
                if $editor->{last_name};

            # "Sabine-Marie Ann-Katrin Kaufmann"
            # "Karl Heinz Müller"
            push @{$editor->{editoren_ansetzung}},
            ...
            if ($first_initial) {
                ...
                # "Müller K"
                push @{$editor->{editoren_ansetzung}},
                    "$editor->{last_name}, $first_initial";
                ...
                # "K Müller"
                push @{$editor->{editoren_ansetzung}},
                    "$first_initial $editor->{last_name}";
