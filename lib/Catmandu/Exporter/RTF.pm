...
sub _add_cite {
    ...
    my $indent;
    if($cite =~ /<div style="text-indent:-25px; padding-left:25px;padding-bottom:0px;">(.*?)<\/div>/){
    	$indent = 1;
    ...
    # - so: hex, it is!
    $cite =~ s/([^\\\\u|\\\\i|\\\\ldblquote|\\\\rdblquote|\\\\li380|\\\\fi\-380|\\\\line|___|\{|\}|\(|\)|[0-9]|\s])/sprintf("\\'%02x",ord($1))/eg;
    $title =~ s/([^\\\\u|\\\\i|\\\\line|___|\{|\}|\(|\)|[0-9]|\s])/sprintf("\\'%02x",ord($1))/eg if $title;
    ...
    my $rtftitle = "{\\field{\\*\\fldinst HYPERLINK http://pub.uni-bielefeld.de/$bag/$pub->{_id}}{\\fldrslt " . $title . "}}\\line " if $title;
