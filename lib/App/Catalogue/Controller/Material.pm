...
my $relations_file = h->config->{lists}->{relations_file};
...
sub update_related_material {
    ...
    my $related_material_record = $pub->{related_material}->{record} if $pub->{related_material}->{record};
...
#    }
    ...
    foreach my $rm (@$related_material_record){
        ...
        if ( $rm->{id} ) {
            ...
            my ($ref) = grep { $_->{relation} eq $rm->{relation} } @$relations_link;
            ...
            if(!$opposite->{related_material} or !$opposite->{related_material}->{record}){
            	...
            	my ($ref) = grep { $_->{relation} eq $rm->{relation} and $_->{id} eq $pub->{_id} } @{$opposite->{related_material}->{record}};
            	if (!$ref){
            		push @{$opposite->{related_material}->{record}}, {id => $pub->{_id}, relation => $op_relation};
            	}
            }
    		...
    		if($hits->{total}){
    			$hits->each(sub {
    				...
    				if($rec->{related_material}){
    					if($rec->{related_material}->{record}){
