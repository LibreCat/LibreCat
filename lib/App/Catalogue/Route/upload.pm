...
prefix '/librecat' => sub {
    ...
    post '/upload' => needs login => sub {
        ...
        if ($file) {
            ...
            my $filepath
                = path(h->config->{tmp_dir}, $tempid, $file->{filename});
    ...
    post '/upload/qae/submit' => needs login => sub {
        ...
        if ($submit_or_cancel eq "Submit") {
            ...
            my $person  = h->get_person(params->{delegate}
            ...
            my $path
                = path(h->config->{tmp_dir}, params->{tempid}, $file_name);
    ...
    post '/thesesupload/submit' => sub {
        ...
        if ($submit_or_cancel eq "Submit") {
            ...
            if (system "-e $temp_file") {
                ...
                my $path = path(h->config->{tmp_dir}, params->{tempid},
                    $file_name);
                update_file($id, $file_name, $path);
    ...
    post '/upload/update' => needs login => sub {
        ...
        my $open_access = params->{access_level}
            && params->{access_level} eq "open_access" ? 1 : 0;
