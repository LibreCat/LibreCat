package LibreCat::Permission::Publication;

use Catmandu::Sane;
use Catmandu;
use Catmandu::Util qw(:is);
use LibreCat qw(publication user);
...
sub _can_do_action {
    ...
    unless (defined($action)) {
        $self->log->fatal("whoops! an action role needs to be filled in");
        return 0;
    }
    ...
    $self->log->debug("id: $id ; opts:" . to_dumper($opts));
    ...
    my $action_permissions = librecat->config->{permissions}->{access}->{$action};
...
sub can_download {
    ...
    my $ip_range = librecat->config->{ip_range};
    my $access;
    my $file_name;
...
sub all_author_types {
    ...
    my $permissions = librecat->config->{permissions}->{access} // {};
