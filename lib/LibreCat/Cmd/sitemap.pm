...
sub command {
    ...
    $bag->select(sub { $_[0]->{status} && $_[0]->{status} eq 'public' })->group(10000)->each(sub {
        my $group = $_[0];
        $n++;
        ...
        $file->say('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">');
        $group->each(sub {
            my $rec  = $_[0];
            my $type = $rec->{type} eq 'researchData' ? 'data' : 'publication';
            my $loc  = "$config->{host}/$type/$rec->{_id}";
            my $mod  = substr($rec->{date_updated} || $rec->{date_created} || $today, 0, 10);
            $file->say("<url><loc>$loc</loc><lastmod>$mod</lastmod><priority>0.9</priority></url>");
        });
        $file->say('</urlset>');
        $file->close;
    });
    ...
    $file->say('<?xml version="1.0" encoding="UTF-8"?>');
    $file->say('<sitemap xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">');
    for my $i (1 .. $n) {
