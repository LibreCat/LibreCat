package LibreCat::App::Search::Route::award;

=head1 NAME

LibreCat::App::Search::Route::award - handling routes for award pages.
    ...
    if ($mailform and $mailform eq "sent") {
        ...
        if ($email and $email =~ /\@.*?\./ and ($message or $preis)) {
            foreach my $recipient (@$receiver) {
                ...
                print MAIL
                    "Subject: Wissenschaftspreise: neuer Vorschlag\n\n";
            ...
            if ($lang and $lang eq "en") {
                $returnhash->{returnmessage}
                    = "Please enter your email address.<br />"
                    if !$email;
                $returnhash->{returnmessage}
                    .= "Please enter a correct email address.\n"
                    if ($email and $email !~ /\@.*?\./);
                $returnhash->{emailerror} = "ERROR"
                    if (!$email or $email !~ /\@.*?\./);
                $returnhash->{returnmessage}
                    .= "Please enter the name of the award in the field 'award name' or 'message'.\n"
                    if (!$message and !$preis);
                $returnhash->{messageerror} = "ERROR"
                    if (!$message and !$preis);
                $returnhash->{awarderror} = "ERROR"
                    if (!$message and !$preis);
            }
            else {
                $returnhash->{returnmessage}
                ...
                $returnhash->{emailerror} = "ERROR"
                    if (!$email or $email !~ /\@.*?\./);
                $returnhash->{returnmessage}
                    .= "Bitte geben Sie unter 'Preisname' oder 'Nachricht' den Namen des Preises ein.<br />"
                    if (!$message and !$preis);
                $returnhash->{messageerror} = "ERROR"
                    if (!$message and !$preis);
                $returnhash->{awarderror} = "ERROR"
    ...
    # Award details page
        ...
        if($hit->{rec_type} ne "record"){
            ...
            my $awardhits = h->award->search(
                ...
                limit     => params->{limit}
                    ||= h->config->{default_searchpage_size},
