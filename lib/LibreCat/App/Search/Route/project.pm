...
get qr{/project/*} => sub {
    if (params->{ttyp} and params->{ttyp} eq "hist_proj") {
        ...
        my $p = {limit => $limit, start => params->{start} ||= 0,};
        ...
        push @{$p->{q}}, "startyear=" . params->{startYear}
            if params->{startYear} && params->{startYear} ne "";
        push @{$p->{q}}, "endyear=" . params->{endYear}
            if params->{endYear} && params->{endYear} ne "";

        push @{$p->{q}}, "department=" . params->{department}
            if params->{department} && params->{department} ne "";

        my $facets = {
            startYear =>
                {terms => {field => 'start_year.exact', size => 100}},
