...
sub test_header {
    my $results = $main::driver->find_element('html/body/header/nav/div[1]/a',
        'xpath');
    is($results->get_text(), '', 'header - Icon link text');
    is($results->get_attribute('title'), 'PUB home', ' - Icon link title');
    is($results->get_attribute('href'),
        'http://demo.librecat.org/', ' - home link');

    $results = $main::driver->find_element(
        'html/body/header/nav/div[2]/ul[1]/li/a', 'xpath');
    ...
    my $hasSearchForm
        = !($main::driver->get_current_url() eq 'http://demo.librecat.org/'
        || $main::driver->get_current_url() eq
        'http://demo.librecat.org/person'
        || $main::driver->get_current_url() eq
        'http://demo.librecat.org/project'
        || $main::driver->get_current_url() eq
        'http://demo.librecat.org/department');

    if (!$hasSearchForm) {
        is(@results, 3,
            'header - helpme ' . $main::driver->get_current_url());
    }
    else {
        is(@results, 4,
            'header - helpme ' . $main::driver->get_current_url());
    ...
    is($results[0]->get_attribute('title'),
        'PUB home', ' - header - helpme 1');
    is(
        $results[1]->get_attribute('title'),
        'Backend<br>start page',
        ' - header - helpme 2'
    );
    is(
        $results[2]->get_attribute('title'),
        'Change language',
        ' - header - helpme 3'
    );
    ...
    if ($hasSearchForm) {
        ...
        is($results[3]->get_attribute('data-placement'),
            'left', ' - header - helpme 4');
        is(
            $results[3]->get_attribute('title'),
            'Search this publication list.',
            ' - header - helpme 4'
        );
    }
    ...
    is(
        ...
        'http://demo.librecat.org/set_language?lang=de',
        ' - header - change language link'
    );

    $results = $main::driver->find_element('glyphicon-log-out', 'class');
    $link
        = $main::driver->find_child_element($results, "./parent::a", 'xpath');
    is(
        $link->get_attribute('href'),
        'http://demo.librecat.org/login',
        ' - header - login link'
    );
}
...
sub test_footer {
    ...
    my $footer
        = $main::driver->find_child_element($results, 'col-sm-5', 'class');
...
sub test_home {
    ...
    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[1]/span[2]', 'xpath');
    is($results->get_text(), '17', 'number of publications');
    is($results->get_attribute('class'), 'statalign', ' - check attribute');
    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[1]/span[1]', 'xpath');
    is($results->get_attribute('class'), 'fa fa-check fa-1x',
        ' - check sign');

    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[2]/span[2]', 'xpath');
    is($results->get_text(), '1', 'number of data publications');
    is($results->get_attribute('class'), 'statalign', ' - check attribute');
    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[2]/span[1]', 'xpath');
    is($results->get_attribute('class'), 'fa fa-check fa-1x',
        ' - check sign');

    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[3]/span[2]', 'xpath');
    is($results->get_text(), '0', 'number of open access publications');
    is($results->get_attribute('class'), 'statalign', ' - check attribute');
    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[3]/span[1]', 'xpath');
    is($results->get_attribute('class'), 'fa fa-check fa-1x',
        ' - check sign');

    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[4]/span[2]', 'xpath');
    is($results->get_text(),             '1584',      'number of projects');
    is($results->get_attribute('class'), 'statalign', ' - check attribute');
    $results = $main::driver->find_element(
        './/*[@id="home"]/div/div/div[2]/div/p[4]/span[1]', 'xpath');
    is($results->get_attribute('class'), 'fa fa-check fa-1x',
        ' - check sign');
    ...
    my @tabs = $results
        = $main::driver->find_child_elements($results, 'li', 'xpath');
    ...
    testTabItem($tabs[0][1], 'http://demo.librecat.org/publication',
        'Publications');
    testTabItem(
        $tabs[0][2],
        'http://demo.librecat.org/data',
        'Data Publications'
    );
    testTabItem($tabs[0][3], 'http://demo.librecat.org/person',  'Authors');
    testTabItem($tabs[0][4], 'http://demo.librecat.org/project', 'Projects');
    testTabItem($tabs[0][5], 'http://demo.librecat.org/department',
        'Departments');

    $results = $main::driver->find_element('h1', 'tag_name');
    is($results->get_text(), 'Publications at LibreCat University',
        'home - title');
...
# test_link($links[7], 'http://demo.librecat.org/publication?cql=department=2685177', 'Fakult�t f�r Biologie'); #'Fakultät für Biologie');
    ...
    test_link(
        $links[8],
        'http://demo.librecat.org/publication?cql=department=10085',
        'University Library'
    );
...
SKIP: {
    ...
    unless ($ENV{SELENIUM_TEST}) {
        skip(
            "No Selenium instance. You may want to run docker run -d -p 4444:4444 selenium/standalone-chrome' and set SELENIUM_TEST to run these tests.",
            5
        );
    }
