...
$user = $users->get_by_username('user1');
is_deeply $user, $user1;
$user = $users->get_by_username('user2');
...
is $users->may($user, 'view', {_type => 'publication', status => 'public'}),
    1;
is $users->may($user, 'view',
...
$user = $users->get_by_username('user3');

# reviewer is also a user
is $users->may($user, 'edit',
    {_type => 'publication', creator => {login => 'user3'}}),
    1;
